...
public class LoginUsersOauth2 {
      ...
      private enum MetricAuth {
            OpenLoginPage("open-login-page"), GetCode("get-code"), GetToken("get-token"), Login("login");
      ...
      public static void main(String[] args) throws Exception {
            ...
            final long max = (maxUsers < 0) ? Long.MAX_VALUE : maxUsers;
            long users = 0;
            for (Map.Entry<Object, Object> user : usersProperties.entrySet()) {
                  ...
                  final String startUrl = baseUrl + "/api/authorize?response_type=code" + "&client_id=" + clientId
                              + "&scope=user:email" + "&redirect_uri=" + redirectUrl + "&state=" + uuid.toString();
                  ...
                  if (!uuid.toString().equals(returnState)) {
                        throw new Exception("Return state (" + returnState + ") should be equal to the request state ("
                                    + uuid.toString() + ")");
                  }
                  ...
                  final String bodyString = "grant_type=authorization_code" + "&client_id=" + clientId + "&code=" + code
